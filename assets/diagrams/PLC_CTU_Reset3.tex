\begin{circuitikz}[
    ...
    alt={Ladder logic diagram bounded by two vertical power rails. The left power rail is highlighted in green. The diagram consists of two active rungs: On the first rung, a line extends from the left rail through a normally open contact labeled Photo1. The green highlight extends through the Photo1 contact up to the next component. Following the contact, the rung connects to a count up counter block labeled CTU with the tag name PartCount. Inside the block, the preset value PRE is set to 10 and the accumulated value ACC is set to 10. Output status lines extend from the right side of the block for enable EN and done DN, with both the EN and DN lines highlighted in green. The main output line connects the block to the right power rail. On the bottom rung, a line extends from the left rail through a normally open contact labeled BatchDone and connects to an oval reset block labeled RES with the tag name PartCount.}
    ]
    ...
    \begin{pgfonlayer}{main}
        ...
        \begin{scope}[
            ...
            % Rung 1
            %=================================================================
            \draw (C-0-0) to[C, l={Photo1}] (C-1-0) -- (C-3-0) pic (CTU) {TON};
            \draw (CTU-OUT) -- (C-5-0);

            % CTU box labels
            \node[anchor=south] at (CTU-TOP) {CTU};
            \node[anchor=north] at (CTU-TOP) {PartCount};
            \node[anchor=west] at (CTU-PRE) {10};
            \node[anchor=west] at (CTU-ACC) {10};
    ...
    \begin{pgfonlayer}{bg}
        ...
        \begin{scope}[
            ...
            \draw (CTU-EN1) -- (CTU-EN2);

            \draw (CTU-DN1) -- (CTU-DN2);
