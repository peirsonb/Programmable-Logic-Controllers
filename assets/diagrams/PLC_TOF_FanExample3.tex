\begin{circuitikz}[
    ...
    alt={Diagram showing a two-rung ladder logic diagram featuring a Timer Off Delay (TOF) instruction labeled FanTimer and an output coil labeled CoolingFan. On the top rung (Rung 1), the MotorOn contact is open and the TOF timer has completed its off-delay timing interval, reaching an Accumulated (ACC) value of 2003 against a Preset (PRE) value of 2000. Because the timer has finished timing out and power flow is off, neither the timer status bits nor the bottom rung (Rung 3) display green power flow highlighting, leaving green highlighting present only on the main left vertical power rail.}
    ]
    ...
    \begin{pgfonlayer}{main}
        ...
        \node[text=BakerBlack, anchor=west] at (T1-ACC) {{\small 2003}};
