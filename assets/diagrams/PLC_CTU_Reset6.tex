\begin{circuitikz}[
    ...
    alt={Ladder logic diagram bounded by two vertical power rails. The left power rail is highlighted in green. The diagram consists of two active rungs: On the first rung, a line extends from the left rail through a normally open contact labeled Photo1 and connects to a count up counter block labeled CTU with the tag name PartCount. Inside the block, the preset value PRE is set to 10 and the accumulated value ACC is set to 0. Output status lines extend from the right side of the block for enable EN and done DN. The main output line connects the block to the right power rail. On the bottom rung, a line extends from the left rail through a normally open contact labeled BatchDone and connects to an oval reset block labeled RES with the tag name PartCount. The entire rung, including the contact and reset block, is highlighted in green through to the right power rail.}
    ]
