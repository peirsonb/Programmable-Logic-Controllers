\begin{circuitikz}[
    ...
    alt={Ladder logic diagram bounded by two vertical power rails. The left power rail is highlighted in green. The diagram consists of one active rung: On the rung, a line extends from the left rail through a normally open contact labeled StartPB. The green highlight extends through the StartPB contact up to the next component. Following the contact, the rung connects to a divide instruction block labeled DIV. Inside the block, Source A is set to tag RunTime with a value of 364, Source B is set to tag AutoTime with a value of 0, and Destination is set to tag UpTime with a value indicated as question marks. The main output line connects the block to the right power rail. Above the DIV block, bold red text reads Major Fault.}
    ]
    ...
    \begin{pgfonlayer}{main}
        ...
        \begin{scope}[
            ...
            % Rung 1
            %=================================================================
            \draw (C-0-0) to[C, l={StartPB}] (C-1-0) -- (C-3-0) pic (DIV) {SrcDest};
            \draw (DIV-OUT) -- (C-5-0);
            ...
            \node[text=BakerADARed, font=\bfseries\Large] at ($(C-2-0)+(1.5,1)$) {Major Fault};
