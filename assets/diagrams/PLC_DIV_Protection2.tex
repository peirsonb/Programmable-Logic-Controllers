\begin{circuitikz}[
    ...
    alt={Ladder logic diagram bounded by two vertical power rails. The left power rail is highlighted in green. The diagram consists of one active rung: On the rung, a line extends from the left rail through a normally open contact labeled StartPB, with the green highlight extending through the contact. Following the contact, the line connects to a Not Equal comparison block labeled NE. Inside the NE block, Source A is set to tag AutoTime with a value of 0, and Source B is set to 0. Following the NE block, the line connects to a divide instruction block labeled DIV. Inside the DIV block, Source A is set to tag RunTime with a value of 364, Source B is set to tag AutoTime with a value of 0, and Destination is set to tag UpTime with a value indicated as question marks. The main output line connects the DIV block to the right power rail. Above the rung, green text reads NE=false, DIV doesn't execute.}
    ]
    ...
    \begin{pgfonlayer}{main}
        ...
        \begin{scope}[
            ...
            % Rung 1
            %=================================================================
            \draw (C-0-0) to[C, l={StartPB}] (C-1-0) pic (NE) {SrcAB};
            \draw (NE-OUT) -- (C-3-0) pic (DIV) {SrcDest};
            \draw (DIV-OUT) -- (C-5-0);

            % DIV box labels
            \node[anchor=south] at (DIV-TOP) {DIV};
            \node[anchor=west] at (DIV-SRCA) {RunTime};
            \node[anchor=west] at (DIV-AVAL) {364};
            \node[anchor=west] at (DIV-SRCB) {AutoTime};
            \node[anchor=west] at (DIV-BVAL) {0};            
            \node[anchor=west] at (DIV-DEST) {UpTime};
            \node[anchor=west] at (DIV-DVAL) {??};

            % NE box labels
            \node[anchor=south] at (NE-TOP) {NE};
            \node[anchor=west] at (NE-SRCA) {AutoTime};
            \node[anchor=west] at (NE-AVAL) {0};
            \node[anchor=west] at (NE-SRCB) {0};
            \node[anchor=west] at (NE-BVAL) {0};

            \node[text=UpNorthGreen, font=\bfseries\Large] at ($(C-2-0)+(1.5,1.5)$) {NE=false, DIV doesn't execute};
