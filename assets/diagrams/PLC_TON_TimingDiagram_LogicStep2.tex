\begin{circuitikz}[
    ...
    alt={Diagram showing a single-rung ladder logic program featuring a Timer On Delay (TON) instruction named My_Timer. The normally open contact Prox1 is energized, allowing power flow across Rung 1. The timer has completed its timing sequence, reaching an Accumulated (ACC) value of 2004 against a Preset (PRE) value of 2000. Green power flow highlighting extends from the left vertical power rail through Prox1 and highlights both the Enable (EN) and Done (DN) bits on the timer block, indicating that the rung condition remains true and the timer has finished timing.}
    ]
