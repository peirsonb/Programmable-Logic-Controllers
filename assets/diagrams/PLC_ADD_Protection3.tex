\begin{circuitikz}[
    ...
    alt={Ladder logic diagram labeled Scan 1, bounded by two vertical power rails. The left power rail is highlighted in green. A single rung extends from the left rail through a normally open contact labeled Start PB and continues through an oval one-shot block labeled ONS with the tag name OneShot. The green highlight extends through both the Start PB contact and the ONS block up to the addition instruction. Following the one-shot block, the rung connects to an addition block labeled ADD. Inside the block, Source A is set to PartCount with a value of 9, Source B is set to 1, and the Destination is set to PartCount with a value of 10. The output line connects the right side of the ADD block to the right power rail.}
    ]
    ...
    \begin{pgfonlayer}{main}
        ...
        \begin{scope}[
            ...
            \node[text=BakerADARed, font=\bfseries\Large] at ($(C-2-0)+(1.5,1)$) {Scan 1};
