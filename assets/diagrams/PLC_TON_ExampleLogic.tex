\begin{circuitikz}[
    ...
    alt={Diagram showing a single-rung ladder logic diagram featuring a Timer On Delay (TON) instruction named My_Timer. On Rung 1, a normally open contact labeled Prox1 is in an open (false) state, preventing power flow to the timer. As a result, the TON instruction block remains inactive with a Preset (PRE) of 2000 and an Accumulated (ACC) value of 0. Green power flow highlighting is present only on the left vertical power rail, confirming that no energy is reaching the timer block or its status bits.}
    ]
