\begin{circuitikz}[
    ...
    alt={Ladder logic diagram bounded by two vertical power rails. The left power rail is highlighted in green. The diagram consists of one active rung. On the rung, a line extends from the left rail directly to a Limit Test comparison block labeled LIMIT. Inside the LIMIT block, Low Limit is set to 20, Test tag is set to BatchCount with a current value of 13, and High Limit is set to 10. Following the LIMIT comparison block, the line connects to an output coil labeled OpenDoor, which connects to the right power rail. No highlight is active on the rung because when Low Limit is greater than High Limit, the instruction tests whether the Test value is less than or equal to High Limit or greater than or equal to Low Limit, which evaluates to false since 13 is neither less than or equal to 10 nor greater than or equal to 20.}
    ]
    ...
    \begin{pgfonlayer}{main}
        ...
        \begin{scope}[
            ...
            % Rung 1
            %=================================================================
            \draw (C-0-0) -- ++(0.5,0) pic (LIMIT) {Limit};
            \draw (LIMIT-OUT) -- (C-4-0) to[esource, l={OpenDoor}] (C-5-0);

            % GT box labels
            \node[anchor=south] at (LIMIT-TOP) {LIMIT};
            \node[anchor=west] at (LIMIT-LO) {20};
            \node[anchor=west] at (LIMIT-LOV) {20};
            \node[anchor=west] at (LIMIT-TEST) {BatchCount};
            \node[anchor=west] at (LIMIT-TESTV) {13};
            \node[anchor=west] at (LIMIT-HI) {10};
            \node[anchor=west] at (LIMIT-HIV) {10};
