\begin{circuitikz}[
    ...
    alt={Ladder logic diagram bounded by two vertical power rails. The left power rail is highlighted in green. The diagram consists of one active rung. On the rung, a line extends from the left rail directly to a Limit Test comparison block labeled LIMIT. Inside the LIMIT block, the Low Limit is set to 10, the Test tag is set to BatchCount with a current value of 8, and the High Limit is set to 20. Following the LIMIT comparison block, the line connects to an output coil labeled OpenDoor, which connects to the right power rail. No highlight is active on the rung because the test condition (10 <= 8 <= 20) evaluates to false.}
    ]
    ...
    \begin{pgfonlayer}{main}
        ...
        \begin{scope}[
            ...
            \node[anchor=west] at (LIMIT-LO) {10};
            \node[anchor=west] at (LIMIT-LOV) {10};
            \node[anchor=west] at (LIMIT-TEST) {BatchCount};
            \node[anchor=west] at (LIMIT-TESTV) {8};
            \node[anchor=west] at (LIMIT-HI) {20};
            \node[anchor=west] at (LIMIT-HIV) {20};
