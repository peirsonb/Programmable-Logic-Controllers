\begin{circuitikz}[
    ...
    alt={Ladder logic diagram labeled Scan 5, bounded by two vertical power rails. The left power rail is highlighted in green. A single rung extends from the left rail through a normally open contact labeled Start PB and continues through an oval one-shot block labeled ONS with the tag name OneShot. The green highlight extends through both the Start PB contact and the ONS block up to the addition instruction. Following the one-shot block, the rung connects to an addition block labeled ADD. Inside the block, Source A is set to PartCount with a value of 10, Source B is set to 1, and the Destination is set to PartCount with a value of 10. The output line connects the right side of the ADD block to the right power rail.}
    ]
    ...
    \begin{pgfonlayer}{main}
        ...
        \begin{scope}[
            ...
            % Rung 1
            %=================================================================
            \draw (C-0-0) to[C, l={StartPB}] (C-1-0) pic (ONS) {ONS};
            \draw (ONS-OUT) -- (C-3-0) pic (ADD) {SrcDest};
            \draw (ADD-OUT) -- (C-5-0);
            ...
            \node[anchor=west] at (ADD-SRCA) {PartCount};
            \node[anchor=west] at (ADD-AVAL) {10};
            \node[anchor=west] at (ADD-SRCB) {1};
            \node[anchor=west] at (ADD-BVAL) {1};            
            \node[anchor=west] at (ADD-DEST) {PartCount};
            \node[anchor=west] at (ADD-DVAL) {10};

            \node[text=BakerADARed, font=\bfseries\Large] at ($(C-2-0)+(1.5,1.5)$) {Scan 5};
