\begin{circuitikz}[
    ...
    alt={Diagram showing a single-rung ladder logic program featuring a Timer On Delay (TON) instruction named My_Timer. The normally open contact Prox1 is energized, allowing power flow to the timer block. The timer is currently active and counting, with an Accumulated (ACC) value of 547 out of a Preset (PRE) value of 2000. Green power flow highlighting extends from the left vertical rail through Prox1 and highlights the Enable (EN) bit of the timer block, indicating that the rung condition is true while the timer actively accumulates time.}
    ]
