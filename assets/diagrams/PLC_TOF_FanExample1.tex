\begin{circuitikz}[
    ...
    alt={Diagram showing a two-rung ladder logic diagram featuring a Timer Off Delay (TOF) instruction labeled FanTimer and an output coil labeled CoolingFan. On the top rung (Rung 1), a normally open contact MotorOn connects to the TOF timer block, which has a Preset (PRE) value of 2000 and an Accumulated (ACC) value of 0. Green power flow highlighting extends from the left vertical rail through the MotorOn contact to highlight both the enable (EN) and done (DN) bits of the timer block, indicating that the rung condition is true while the motor is running. On the bottom rung (Rung 3), a normally open contact labeled My_Timer.DN energizes the CoolingFan output coil, with green power flow highlighting spanning across the entire rung to show that the coil is currently energized.}
    ]
