\begin{circuitikz}[
    ...
    alt={Ladder logic diagram titled Toggle Logic - First Scan featuring two rungs bounded by vertical power rails with green highlighting indicating active power flow across both main rungs. The top rung contains an examine if closed contact labeled AutoPB in series with a one shot instruction labeled OneShot leading to an output coil labeled AutoSS, with a green highlight spanning the entire rung. The bottom rung consists of a main branch with an examine if closed contact labeled AutoSS in series with an examine if open contact labeled Auto leading to an output coil labeled Auto, with a green highlight tracing directly through this upper branch. A lower parallel branch containing an examine if open contact labeled AutoSS in series with an examine if closed contact labeled Auto wraps around the top contacts but remains unhighlighted.}
    ]
    ...
    \begin{pgfonlayer}{main}
        ...
        \begin{scope}[
            ...
            \draw ($(C-0-1)+(0.25,0)$) -- ++(0,-2) to[vC, l={AutoSS}] (C-1-2) to[C, l={Auto}] ($(C-2-2)-(0.25,0)$) -- ++(0,2);
