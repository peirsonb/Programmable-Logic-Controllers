\begin{circuitikz}[
    ...
    alt={Diagram showing a two-rung ladder logic diagram where Rung 1 features a Timer On Delay instruction named My_Timer and Rung 3 contains a contact labeled My_Timer.DN controlling an output coil for Valve1. On Rung 1, the energized Prox1 contact provides power flow to the timer, which has finished timing with an Accumulated value of 2004 relative to a Preset value of 2000, illuminating both its Enable and Done status bits in green. Because the Done bit is true, the My_Timer.DN contact on Rung 3 is closed, allowing green power flow to extend entirely across Rung 3 from the left rail to energize the Valve1 output coil.}
    ]
    ...
    \begin{pgfonlayer}{main}
        ...
        \begin{scope}[
            ...
            % Rung 1
            %=================================================================
            \draw (C-0-0) to[C, l={Prox1}] (C-1-0) -- (C-3-0) pic (T1) {TON};
            \draw (T1-OUT) -- (C-5-0);

            % TON labels
            \node[anchor=south] at (T1-TOP) {TON};
            \node[anchor=north] at (T1-TOP) {My\_Timer};
            \node[anchor=west] at (T1-PRE) {2000};
            \node[anchor=west] at (T1-ACC) {2004};

            %=================================================================
            % Rung 3 (timer box takes 2 rungs)
            %=================================================================
            \draw (C-0-2) to[C, l={My\_Timer.DN}] (C-1-2) -- (C-4-2) to[esource, l={Valve1}] (C-5-2);
