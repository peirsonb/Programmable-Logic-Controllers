\begin{circuitikz}[
    ...
    alt={Ladder logic diagram titled Toggle Logic - Second Button Press featuring two rungs bounded by vertical power rails with green highlighting indicating active power flow. The top rung contains an examine if closed contact labeled AutoPB in series with a one shot instruction labeled OneShot leading to an output coil labeled AutoSS, with a green highlight spanning the entire length of the top rung. The bottom rung contains a top branch with an examine if closed contact labeled AutoSS in series with an examine if open contact labeled Auto leading to an output coil labeled Auto, wrapped over a lower branch with an examine if open contact labeled AutoSS in series with an examine if closed contact labeled Auto, with both branches and the output coil on the bottom rung remaining completely unhighlighted.}
    ]
